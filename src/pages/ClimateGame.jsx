...
import { 
  ...
  CheckCircle, XCircle, Target, Award, Flame, Sparkles
} from 'lucide-react';
...
export default function ClimateGame() {
  const [gameState, setGameState] = useState('menu'); // menu, playing, results, ai-setup
  ...
  useEffect(() => {
    ...
    if (isTimerActive && timeLeft > 0) {
      ...
      if (gameType === 'sorting') {
        // End the sorting game when timer runs out
        setIsTimerActive(false);
        setGameState('results');
      } else {
        // Handle timeout for quiz game
        handleTimeout();
      }
    }
    return () => clearInterval(timer);
  }, [isTimerActive, timeLeft, gameType]);
  ...
  const startGame = (type = 'quiz') => {
    // If quiz type, show AI setup first
    if (type === 'quiz') {
      setGameType(type);
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-8 mb-12">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="card-3d bg-gradient-to-br from-purple-600/30 to-pink-700/30 rounded-3xl p-8 border border-purple-500/50 text-left hover:border-purple-400 transition-all group relative overflow-hidden"
              >
                <div className="absolute top-2 right-2">
                  <Badge className="bg-purple-500 text-white border-none text-xs">AI POWERED</Badge>
                </div>
                <div className="text-6xl mb-4 group-hover:scale-110 transition-transform">✨</div>
                <h3 className="font-orbitron text-2xl font-bold text-purple-400 mb-2">AI Climate Quiz</h3>
                <p className="text-gray-400 mb-4">Generate custom questions on any climate topic using AI!</p>
  ...
  // AI Quiz Setup Screen
  if (gameState === 'ai-setup') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900">
        <header className="border-b border-slate-800 bg-slate-900/50 backdrop-blur-xl">
          <div className="max-w-7xl mx-auto px-4 py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xl font-bold text-white">AI Quiz Generator</span>
              </div>
              <div className="w-24" />
            </div>
          </div>
        ...
        <main className="max-w-7xl mx-auto px-4 py-12">
          <AIQuizGenerator
            onQuestionsGenerated={(questions) => {
              // Replace quiz questions with AI-generated ones
  ...
  if (gameState === 'results') {
    ...
    let correctAnswers, totalQuestions, percentage;
    
    if (gameType === 'sorting') {
      correctAnswers = sortingState.correctCount;
      totalQuestions = sortingState.items.length;
        ...
        <main className="max-w-2xl mx-auto px-4 py-12">
          ...
          >
            ...
            <p className="text-slate-400">
              You've completed {gameType === 'sorting' ? 'Waste Sorter' : 'Climate Quiz'}!
            </p>
          </motion.div>
